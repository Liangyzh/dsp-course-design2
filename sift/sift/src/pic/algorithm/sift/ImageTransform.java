...
//			 System.out.println(keyPoints.get(1));
			 if(null!=imagePoint){
				...
				/*	BufferedImage bimg=getP2P(tempImage, vector, null, null);
					...
					int savepointx[]=new int[imagePoint.size()];
					int savepointy[]=new int[imagePoint.size()];
					int counti=0;
					///如果该图像是求了极值点的图像
...
//		int length=pSet.size()-1;
		for(int i:pSet){
			...
			//List<MyPoint> list=resultMap.get(i);
...
//			System.out.println(mp1.getPreX()+"  "+mp1.getPreY());
//			System.out.println(minED/sMinED+"哈哈："+minED);
